...
import {fileRows} from './view-model.js';
...
export default class TransistPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        for (const [name, title, icon] of [['active', 'Active Files', 'folder-symbolic'], ['history', 'Recently Deleted', 'document-open-recent-symbolic']]) {
            ...
            const files = new Adw.PreferencesGroup({title: name === 'active' ? 'Your temporary collection' : 'Recovery history', description: name === 'active' ? 'Files expire after five hours unless kept permanently.' : 'Restore within seven days for a fresh five-hour timer.'});
            page.add(files);
            window.add(page);
        ...
        const captureRow = new Adw.ActionRow({title: 'Store screenshots in _transist', subtitle: 'Move new images from the source folder after 30 seconds without changes. Existing images stay where they are.', use_markup: false});
        ...
        for (const [title, subtitle] of [
            ...
            ['Seven-day recovery', 'Recovery copies use disk space and are permanently removed after seven days. Restoring or making a file temporary starts a fresh five hours.'],
