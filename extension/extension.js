...
export default class Transit extends Extension {
    enable() {
        ...
        const destination = GLib.build_filenamev([GLib.get_home_dir(), '_transit']);
        ...
        this._indicator = new PanelMenu.Button(0.0, 'Transit');
        this._indicator.add_child(new St.Icon({
            icon_name: 'document-open-recent-symbolic',
            style_class: 'system-status-icon',
        }));
        const title = new PopupMenu.PopupMenuItem('Transit · files for now', {reactive: false});
        ...
        this._indicator.menu.addAction('Open _transit folder', () => this._openFolder());
        Main.panel.addToStatusArea(this.uuid, this._indicator);
    }

    _openFolder() {
        const uri = Gio.File.new_for_path(GLib.build_filenamev([GLib.get_home_dir(), '_transit'])).get_uri();
        Gio.AppInfo.launch_default_for_uri_async(uri, null, null, (_source, result) => {
            try {
                Gio.AppInfo.launch_default_for_uri_finish(result);
            } catch (error) {
                if (this._indicator)
                    Main.notify('Transit', error.message);
            }
        });
    }

    _resumeAfterFolderRemoval() {
        const marker = Gio.File.new_for_path(GLib.build_filenamev([GLib.get_user_data_dir(), 'transit', 'folder-removal-requested']));
        if (!marker.query_exists(null))
            return;
        try {
            marker.delete(null);
            const process = Gio.Subprocess.new(['systemctl', '--user', 'start', 'transit.service'], Gio.SubprocessFlags.NONE);
            process.wait_check_async(null, (source, result) => {
                try {
                    source.wait_check_finish(result);
                } catch (error) {
                    Main.notify('Transit', `Could not restart cleanup: ${error.message}`);
                }
            });
        } catch (error) {
            Main.notify('Transit', error.message);
