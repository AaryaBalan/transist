...
class InstallerTests(unittest.TestCase):
    ...
    def test_install_launcher_and_uninstall_preserve_user_data(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            desktop = home / '.local/share/applications/io.github.transit.App.desktop'
            old_gui = home / '.local/share/transit-app/transit/gui.py'
            old_theme = home / '.local/share/transit-app/transit/theme.py'
            for path in (desktop, old_gui, old_theme):
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text('old app')
            keep = home / '_transit/keep.txt'
            ...
            try:
                ...
                self.assertEqual(json.loads(result.stdout)['folder'], str(home / '_transit'))
                ...
                unit = home / '.config/systemd/user/transit.service'
                ...
                guard = home / '.local/share/nautilus-python/extensions/transit_guard.py'
                self.assertTrue(guard.is_file())
                self.assertTrue(guard.with_name('transit_guard_native.so').is_file())
                # Uninstall clears real tracked recovery bytes, but keeps active files.
                (home / '_transit/past.txt').write_text('old recovery')
                clock = [1_800_000_000]
                store = Store(home=home, data=home / '.local/share/transit', clock=lambda: clock[0])
                ...
                recovery_path = home / '_transit/.transit-recovery' / recovery['id']
                ...
                self.assertFalse(guard.with_name('transit_guard_native.so').exists())
            finally:
                sys.path[:] = old_path
