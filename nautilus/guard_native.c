 ...
 * typelib by transist_guard.py; opaque GIO objects are never dereferenced here. */
...
void transist_set_enabled(int value) { atomic_store(&enabled, value); }
unsigned transist_removal_attempts(void) { return atomic_load(&removal_attempts); }

static int prepare_removal(void) {
    atomic_fetch_add(&removal_attempts, 1);
    if (!launcher || access(launcher, F_OK) != 0)
        return 1;
    char *argv[] = {"systemctl", "--user", "stop", "transist.service", NULL};
    pid_t pid;
...
static int dispatch(int index, void *file, void *cancellable, void *error) {
    ...
    const char *message = "Deleting _transist is restricted. Disable the Transist extension first.";
    if (state == 0) {
        if (prepare_removal())
            return originals[index](file, cancellable, error);
        message = "Transist could not stop cleanup before folder removal. Please try again.";
    } else if (state < 0) {
        message = "Transist could not verify that it is disabled. Please try again.";
    }
...
int transist_install(void *file, void **delete_slot, void **trash_slot,
